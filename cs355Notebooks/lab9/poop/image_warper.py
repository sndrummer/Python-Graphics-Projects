...
from bilinear_interpolate import BilinearInterpolation
...
class ImgWarper:
    ...
    def warp(self, from_points, to_points):
        ...
        print
        'Deform: ' + str(t1 - t0) + 'ms; interpolation: ' + str(t2 - t1) + 'ms'
        return new_img
...
class AffineDeformation:
    ...
    def move_point(self, point):
        ...
        for i in range(self.n):
            self.A[i] = point.subtract(p_average).multiply(mat).get_dot_p(self.pRelative[i]) * \
                        self.w[i]
